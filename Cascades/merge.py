...
class Parent(Base):
    ...
    children = relationship(  #relationship is used to link bet 2 tables 
        ...
        cascade="all,merge"  # add/update child automatically
    )
...
# Detach parent from session
session.expunge(p4)

# Now merge parent back
session.merge(p4)  # child is also merged automatically because of cascade="merge"
session.commit()
